...
public abstract class AIZombieState : AIState
{
    ...
    protected virtual bool ColliderIsVisible(Collider other, out RaycastHit hitInfo, int playerLayerMask)
    {
        ...
        RaycastHit[] hits = Physics.RaycastAll(head,direction.normalized,_zombieStateMachine.sensorRadius * _zombieStateMachine.sight,_playerLayerMask );

        float closestColliderDistance = float.MaxValue;
        Collider closestCollider = null;
        for (int i = 0; i < hits.Length; i++)
        {
            if(hits[i].distance < closestColliderDistance)
            {
                if (hits[i].transform.gameObject.layer == _bodyPartLayer)
                {
                    if(_stateMachine != GameSceneManager.instance.GetAIStateMachine(hits[i].rigidbody.GetInstanceID()))
                    {
                        closestColliderDistance = hits[i].distance;
